...
\def\pgfkeysdefargopt#1#2#3{%
  ...
  \long\def\pgfkeys@temp[##1]##2\pgfeov{#3}%
...
\nbkeys{
  canvas start/.code={%
   ...
   foldmark/.code={
     \ifoddpage
       \draw[gray] (current page.west) -- coordinate[pos=0.5](foldmark) ++(east:#1);
     \else
       \draw[gray] (current page.east) -- coordinate[pos=0.5](foldmark) ++(west:#1);
     \fi
   },
   foldmark/.default=8mm,
   /tikz/center xshift/.style={
     /tikz/xshift/.expanded=\ifoddpage#1\else-1*#1\fi
   },
   %
   punchmark/.style={
   ...
   % Space Fill Elements
   fill elem/.code={
     \node[fit=(top-elem.south west) (bottom-elem.north east),outer sep=0pt,inner sep=0pt,
           name=top-elem, alias=elem,
          /notebook/elem style/.cd, @top=true,#1] {};
   },
   every pattern/.style={pattern color=gray},
   % Align Pattern to given anchor
   /pgf/pattern keys/align/.code args={(#1.#2)}{
     \pgfpointanchor{#1}{#2}%
     \pgf@xa=\pgf@x  \pgf@ya=\pgf@y%
     \pgfpointanchor{current page}{north west}%
     \pgf@xb=\pgf@x\pgf@yb=\pgf@y%
     \pgfmathsetmacro{\nb@x}{\the\pgf@xa-\the\pgf@xb}%
     \pgfmathsetmacro{\nb@y}{\the\pgf@ya-\the\pgf@yb+\pgfsys@thepageheight}%
     \pgfkeys{/pgf/pattern keys/.cd, xshift=\nb@x,yshift=\nb@y}%
   },
   /pgf/pattern keys/align/.default={(elem.north west)},
   elem style/pattern/.style={
     /tikz/append after command={
       \pgfextra{
         \path [#1] (\tikzlastnode.north east) rectangle (\tikzlastnode.south west);
       }
     },
   },
   elem style/dotted/.style={
     pattern={/notebook/every pattern,pattern={Dots[distance=5mm,radius=.8pt,align,#1]}}
   },
   elem style/squares/.style={
     pattern={/notebook/every pattern,pattern={Hatch[distance=5mm,align,#1]}},
   },
   elem style/line distance/.store in=\nb@es@linedistance,
   elem style/line distance=8.435mm,
   elem style/lines/.style={
     pattern={/notebook/every pattern,pattern={Lines[distance=\nb@es@linedistance,align,#1]}},
   },
 }
 \nbkeys{
   % Elements that contain a TiKz picture
   elem/.code 2 opt={}{
     \begin{lrbox}{\notebookelembox}%
     ...
     \end{lrbox}
     \node[/notebook/elem style/.cd,
           outer sep=0,inner sep=0pt,
           top, alias=elem,/notebook/every border,#1] {\usebox{\notebookelembox}};
    },
   minipage/.code 2 opt={}{
      \begin{lrbox}{\notebookelembox}%
        \begin{pgfinterruptpicture}%
          \begin{minipage}{\dimexpr\elemwidth-2\pgflinewidth\relax}%
            #2%
          \end{minipage}%
        \end{pgfinterruptpicture}
      \end{lrbox}
     \node[/notebook/elem style/.cd,
           outer sep=0,inner sep=0pt,top, alias=elem, /notebook/every border,#1] {\usebox{\notebookelembox}};
  },
  space/.style 2 opt={}{
    elem={[#1] \path ($(0,0)+(up:#2)$);}
  },
  elem lines/.style 2 opt={}{
    space={[#1,lines,draw]\nb@es@linedistance*#2}
  },
}
\nbkeys{
  ...
  draw/.style={
    /tikz/yshift/.expanded=\ifelem@top\pgflinewidth\else-\pgflinewidth\fi,
    /tikz/draw=#1,
    append after command={
      \pgfextra{
  ...
  time code raster/.store in=\nb@tc@raster,
  time code raster=5mm,
  time code distance/.store in=\nb@tc@space,
  time code distance=2,
  time codes/.code={
    \fill[gray!30] (elem.north west)
                   -- coordinate[pos=0.5,yshift=-\nb@tc@raster/2](@)
                   ++(east:\nb@tc@raster) rectangle (elem.south west);
    \path (@)
       foreach \t in {#1} {
         node {\color{gray}\scriptsize \t}
         -- ++(down:\nb@tc@space*\nb@tc@raster)
       };
  },
  time codes/.default={7,...,23},
  ...
  columns/.style 2 opt={draw}{
    elem={[#1]{
      ...
      \foreach \field/\cols in {#2} {
        \node[outer sep=0, anchor=north west,align=left,minimum width=(\elemwidth)/6*\cols,at=(@@.north east),
              /notebook/every column/.try]  (@@) {\strut};
...
\pgfkeys{
  /notebook/elem style/crosses/.style={
    pattern={
...
\pgfkeys{
  ...
  /notebook/calendar/past/.style={
    append after command={
      \pgfextra{
        \draw[thick] (\tikzlastnode.center) -- +(45:1.5ex) -- +(225:1.5ex);
      }
    },
  },
  ...
  /notebook/calendar/every calendar/.style={
    ...
    every day/.append style={text width=2.3ex,align=center},
    day letter headings,
  },
  ...
  /notebook/calendar/.style 2 opt={draw}{
    elem=[{inner sep=3pt,#1}]{
      ...
      \def\nb@ifcal{
          ...
          if (at least=\nb@startdate) {}
          else [nodes={align=right,/notebook/calendar/past}]
...
\csdef{nb@weekday@5}{Sa}
\csdef{nb@weekday@6}{So}
...
\pgfkeys{
  /notebook/month list/.code 2 args={
    ...
    \def\notebook@month@day##1##2##3{
      \pgfkeys{
        /notebook/.cd,
        @wd 5/.style={font=\bfseries,fill=gray!30},
        @wd 6/.style={font=\bfseries,fill=gray!30},
        every column/.style={/notebook/@wd ##3/.try},
        columns={%
