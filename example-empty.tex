\documentclass[12pt,twoside]{article}
\usepackage[a5paper,margin=8mm,bindingoffset=7mm]{geometry}
\usepackage{tikz}
\usetikzlibrary{notebook}

\begin{document}
\nbkeys{
  every canvas/.style={/notebook/.cd,
    foldmark,punchmarks={65mm/2,65mm/2+45mm},logo
  },
  logo/.code={
    \node[anchor=west,rotate=90] at (foldmark) {\color{gray}\scriptsize TU Braunschweig};
    \node[anchor=east,rotate=90] at (foldmark) {\color{gray}\scriptsize Christian Dietrich};
  },
  my dotted/.style={
    fill elem={dotted},
    time codes,
    label={north east}{Notes},
    vline={[gray,dash pattern=on 2.5mm off 2.5mm, dash phase=-1.25mm, line width=0.5\pgflinewidth]20mm},
  },
  my squares/.style={
    fill elem={squares},
    label={north east}{Notes},
    vline={[gray,ultra thick]15mm},
  },
  my lines/.style={
    fill elem={lines},
    label={north east}{Notes},
    vline=15mm,
  },
}

\notebookpage{my dotted}
\notebookpage{my squares}
\notebookpage{my squares}
\notebookpage{my lines}
\notebookpage{my lines}
\end{document}
